...
## [](#load-wnut-17-dataset) Load WNUT 17 dataset
...
You’re ready to start training your model now! Load DistilBERT with [AutoModelForTokenClassification](/docs/transformers/v5.8.0/en/model_doc/auto#transformers.AutoModelForTokenClassification) along with the number of expected labels, and the label mappings:
...
> For a more in-depth example of how to finetune a model for token classification, take a look at the corresponding [PyTorch notebook](https://colab.research.google.com/github/huggingface/notebooks/blob/main/examples/token_classification.ipynb).
...
The simplest way to try out your finetuned model for inference is to use it in a [pipeline()](/docs/transformers/v5.8.0/en/main_classes/pipelines#transformers.pipeline). Instantiate a `pipeline` for NER with your model, and pass your text to it:
...
[←Text classification](/docs/transformers/tasks/sequence_classification) [Question answering→](/docs/transformers/tasks/question_answering)

[Token classification](#token-classification)[Load WNUT 17 dataset](#load-wnut-17-dataset)[Preprocess](#preprocess)[Evaluate](#evaluate)[Train](#train)[Inference](#inference)
