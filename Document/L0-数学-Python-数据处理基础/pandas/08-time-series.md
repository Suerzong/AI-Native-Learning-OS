...
# Time series / date functionality[#](#time-series-date-functionality "Link to this heading")
...
## Timestamps vs. time spans[#](#timestamps-vs-time-spans "Link to this heading")
...
## Converting to timestamps[#](#converting-to-timestamps "Link to this heading")
...
### Providing a format argument[#](#providing-a-format-argument "Link to this heading")
...
### Assembling datetime from multiple DataFrame columns[#](#assembling-datetime-from-multiple-dataframe-columns "Link to this heading")
...
### Invalid data[#](#invalid-data "Link to this heading")
...
### Epoch timestamps[#](#epoch-timestamps "Link to this heading")
...
The `unit` parameter does not use the same strings as the `format` parameter that was discussed [above](#timeseries-converting-format). The available units are listed on the documentation for [`pandas.to_datetime()`](../reference/api/pandas.to_datetime.html#pandas.to_datetime "pandas.to_datetime").
...
### From timestamps to epoch[#](#from-timestamps-to-epoch "Link to this heading")
...
### Using the `origin` parameter[#](#using-the-origin-parameter "Link to this heading")
...
## Generating ranges of timestamps[#](#generating-ranges-of-timestamps "Link to this heading")
...
Convenience functions like `date_range` and `bdate_range` can utilize a variety of [frequency aliases](#timeseries-offset-aliases):
...
### Custom frequency ranges[#](#custom-frequency-ranges "Link to this heading")
...
## Timestamp limitations[#](#timestamp-limitations "Link to this heading")
...
### Partial string indexing[#](#partial-string-indexing "Link to this heading")
...
### Slice vs. exact match[#](#slice-vs-exact-match "Link to this heading")
...
### Exact indexing[#](#exact-indexing "Link to this heading")
...
### Truncating & fancy indexing[#](#truncating-fancy-indexing "Link to this heading")
...
## Time/date components[#](#time-date-components "Link to this heading")
...
## DateOffset objects[#](#dateoffset-objects "Link to this heading")
...
### Parametric offsets[#](#parametric-offsets "Link to this heading")
...
### Using offsets with `Series` / `DatetimeIndex`[#](#using-offsets-with-series-datetimeindex "Link to this heading")
...
### Custom business days[#](#custom-business-days "Link to this heading")
...
Holiday calendars can be used to provide the list of holidays. See the [holiday calendar](#timeseries-holiday) section for more information.
...
### Business hour[#](#business-hour "Link to this heading")
...
### Custom business hour[#](#custom-business-hour "Link to this heading")
...
### Offset aliases[#](#offset-aliases "Link to this heading")
...
### Period aliases[#](#period-aliases "Link to this heading")
...
### Combining aliases[#](#combining-aliases "Link to this heading")
...
### Anchored offsets[#](#anchored-offsets "Link to this heading")
...
### Anchored offset semantics[#](#anchored-offset-semantics "Link to this heading")
...
### Holidays / holiday calendars[#](#holidays-holiday-calendars "Link to this heading")
...
## Time Series-related instance methods[#](#time-series-related-instance-methods "Link to this heading")

### Shifting / lagging[#](#shifting-lagging "Link to this heading")
...
The `shift` method accepts a `freq` argument which can accept a `DateOffset` class or other `timedelta`-like object or also an [offset alias](#timeseries-offset-aliases).
...
### Frequency conversion[#](#frequency-conversion "Link to this heading")
...
### Filling forward / backward[#](#filling-forward-backward "Link to this heading")
...
### Converting to Python datetimes[#](#converting-to-python-datetimes "Link to this heading")
...
## Resampling[#](#resampling "Link to this heading")
...
### Sparse resampling[#](#sparse-resampling "Link to this heading")
...
### Iterating through groups[#](#iterating-through-groups "Link to this heading")
...
### Use `origin` or `offset` to adjust the start of the bins[#](#use-origin-or-offset-to-adjust-the-start-of-the-bins "Link to this heading")
...
### Backward resample[#](#backward-resample "Link to this heading")
...
## Time span representation[#](#time-span-representation "Link to this heading")
...
### Period[#](#period "Link to this heading")
...
### PeriodIndex and period_range[#](#periodindex-and-period-range "Link to this heading")
...
### Period dtypes[#](#period-dtypes "Link to this heading")

`PeriodIndex` has a custom `period` dtype. This is a pandas extension dtype similar to the [timezone aware dtype](#timeseries-timezone-series) (`datetime64[ns, tz]`).

The `period` dtype holds the `freq` attribute and is represented with `period[freq]` like `period[D]` or `period[M]`, using [frequency strings](#timeseries-period-aliases).
...
### PeriodIndex partial string indexing[#](#periodindex-partial-string-indexing "Link to this heading")
...
### Frequency conversion and resampling with PeriodIndex[#](#frequency-conversion-and-resampling-with-periodindex "Link to this heading")
...
## Converting between representations[#](#converting-between-representations "Link to this heading")
...
## Representing out-of-bounds spans[#](#representing-out-of-bounds-spans "Link to this heading")

If you have data that is outside of the `Timestamp` bounds, see [Timestamp limitations](#timeseries-timestamp-limits), then you can use a `PeriodIndex` and/or `Series` of `Periods` to do computations.
...
## Time zone handling[#](#time-zone-handling "Link to this heading")
...
### Working with time zones[#](#working-with-time-zones "Link to this heading")
...
### Fold[#](#fold "Link to this heading")
...
### Ambiguous times when localizing[#](#ambiguous-times-when-localizing "Link to this heading")
...
### Nonexistent times when localizing[#](#nonexistent-times-when-localizing "Link to this heading")
...
### Time zone Series operations[#](#time-zone-series-operations "Link to this heading")
...
[ __ previous Windowing operations ](window.html "previous page") [ next Time deltas __](timedeltas.html "next page")
...
__On this page
  ...
  * [Converting to timestamps](#converting-to-timestamps)
    * [Providing a format argument](#providing-a-format-argument)
    * [Assembling datetime from multiple DataFrame columns](#assembling-datetime-from-multiple-dataframe-columns)
    * [Invalid data](#invalid-data)
    * [Epoch timestamps](#epoch-timestamps)
    * [From timestamps to epoch](#from-timestamps-to-epoch)
    * [Using the `origin` parameter](#using-the-origin-parameter)
  * [Generating ranges of timestamps](#generating-ranges-of-timestamps)
    * [Custom frequency ranges](#custom-frequency-ranges)
  * [Timestamp limitations](#timestamp-limitations)
  * [Indexing](#indexing)
    * [Partial string indexing](#partial-string-indexing)
    * [Slice vs. exact match](#slice-vs-exact-match)
    * [Exact indexing](#exact-indexing)
    * [Truncating & fancy indexing](#truncating-fancy-indexing)
  * [Time/date components](#time-date-components)
  * [DateOffset objects](#dateoffset-objects)
    * [Parametric offsets](#parametric-offsets)
    * [Using offsets with `Series` / `DatetimeIndex`](#using-offsets-with-series-datetimeindex)
    * [Custom business days](#custom-business-days)
    * [Business hour](#business-hour)
    * [Custom business hour](#custom-business-hour)
    * [Offset aliases](#offset-aliases)
    * [Period aliases](#period-aliases)
    * [Combining aliases](#combining-aliases)
    * [Anchored offsets](#anchored-offsets)
    * [Anchored offset semantics](#anchored-offset-semantics)
    * [Holidays / holiday calendars](#holidays-holiday-calendars)
  * [Time Series-related instance methods](#time-series-related-instance-methods)
    ...
    * [Filling forward / backward](#filling-forward-backward)
    * [Converting to Python datetimes](#converting-to-python-datetimes)
  * [Resampling](#resampling)
    * [Basics](#basics)
    * [Upsampling](#upsampling)
    * [Sparse resampling](#sparse-resampling)
    * [Aggregation](#aggregation)
    * [Iterating through groups](#iterating-through-groups)
    * [Use `origin` or `offset` to adjust the start of the bins](#use-origin-or-offset-to-adjust-the-start-of-the-bins)
    * [Backward resample](#backward-resample)
  * [Time span representation](#time-span-representation)
    * [Period](#period)
    * [PeriodIndex and period_range](#periodindex-and-period-range)
    * [Period dtypes](#period-dtypes)
    * [PeriodIndex partial string indexing](#periodindex-partial-string-indexing)
    * [Frequency conversion and resampling with PeriodIndex](#frequency-conversion-and-resampling-with-periodindex)
  ...
  * [Time zone handling](#time-zone-handling)
    ...
    * [Fold](#fold)
    * [Ambiguous times when localizing](#ambiguous-times-when-localizing)
    * [Nonexistent times when localizing](#nonexistent-times-when-localizing)
    * [Time zone Series operations](#time-zone-series-operations)
