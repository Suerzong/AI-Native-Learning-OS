...
Gemini 模型从底层开始就是多模态的，无需训练专门的 ML（机器学习）模型即可解锁广泛的图像处理和计算机视觉任务，包括但不限于图像描述、分类和视觉问答。

除了通用多模态能力外，Gemini 模型还通过额外训练为特定场景（如[目标检测](<#object-detection>)）提供**增强的准确性**。

## 向 Gemini 传递图像

你可以使用两种方法向 Gemini 提供图像输入：
...
The latest model versions are specifically trained improve accuracy of specialized tasks in addition to generic capabilities, like enhanced [object detection](<#object-detection>).
