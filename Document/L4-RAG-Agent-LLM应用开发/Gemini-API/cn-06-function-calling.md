...
[ 获取 API 密钥 ](<https://aistudio.google.com/apikey>) [ Cookbook ](<https://github.com/google-gemini/cookbook>) [ Community ](<https://discuss.ai.google.dev/c/gemini-api/>) 登录
...
[ ![Gemini API](https://ai.google.dev/_static/googledevai/images/gemini-api-logo.svg) ](</>)
  ...
  * 开始使用
    ...
    * [开始使用 using the GenAI SDK](</gemini-api/docs/live-api/get-started-sdk>)
    * [开始使用 using raw WebSockets](</gemini-api/docs/live-api/get-started-websocket>)
  ...
  * Logs and datasets
    ...
    * [开始使用 with logs](</gemini-api/docs/logs-datasets>)
...
函数调用让你可以将模型连接到外部工具和 API。模型不再生成文本响应，而是决定何时调用特定函数并提供执行现实世界操作所需的参数。这使模型能够充当自然语言与现实世界操作和数据之间的桥梁。 函数调用有 3 个主要用例：

  * **增强知识：** 从数据库、API 和知识库等外部来源获取信息。
  * **扩展能力：** 使用外部工具执行计算并扩展模型的局限性，例如使用计算器或创建图表。
  * **执行操作：** 使用 API 与外部系统交互，如安排预约、创建发票、发送邮件或控制智能家居设备。
...
This process can be repeated over multiple turns, allowing for complex interactions and workflows. The model also supports calling multiple functions in a single turn ([parallel function calling](<#parallel_function_calling>)), in sequence ([compositional function calling](<#compositional_function_calling>)), and with built-in Gemini tools ([multi-tool use](<#native-tools>)).
...
**Note:** This feature is available for [Gemini 3](</gemini-api/docs/gemini-3>) series models.
...
[Gemini 3.1 Flash-Lite Preview](</gemini-api/docs/models/gemini-3.1-flash-lite-preview>) | ✔️ | ✔️ | ✔️
[Gemini 3 Flash Preview](</gemini-api/docs/models/gemini-3-flash-preview>) | ✔️ | ✔️ | ✔️
...
[Gemini 2.0 Flash](</gemini-api/docs/models/gemini-2.0-flash>) | ✔️ | ✔️ | ✔️
