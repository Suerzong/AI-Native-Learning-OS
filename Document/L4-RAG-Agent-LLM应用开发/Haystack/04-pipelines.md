...
🔍Search documentation...
  ...
  * [Haystack Concepts](</docs/concepts-overview>)
    ...
    * [Pipelines](</docs/pipelines>)
      ...
      * [Creating Pipelines](</docs/creating-pipelines>)
      * [Serializing Pipelines](</docs/serialization>)
      * [Visualizing Haystack Pipelines](</docs/visualizing-pipelines>)
      * [Debugging Pipelines](</docs/debugging-pipelines>)
      * [Pipeline Breakpoints](</docs/pipeline-breakpoints>)
      * [Pipeline Loops](</docs/pipeline-loops>)
      * [AsyncPipeline](</docs/asyncpipeline>)
      * [Smart Pipeline Connections](</docs/smart-pipeline-connections>)
...
### Agentic Pipelines[​](<#agentic-pipelines> "Direct link to Agentic Pipelines")
...
![Pipeline architecture diagram showing components arranged in parallel branches that converge into a single pipeline flow](/img/83f686b-Pipeline_Illustrations_1_1.png)
...
See [Pipeline Loops](</docs/pipeline-loops>) for a deeper explanation of how loops are executed, how they terminate, and how to use them safely.

![Pipeline architecture diagram illustrating a feedback loop where output from later components loops back to earlier components](/img/2390eea-Pipeline_Illustrations_1_2.png)

### Async Pipelines[​](<#async-pipelines> "Direct link to Async Pipelines")
...
Find out more in our [AsyncPipeline](</docs/asyncpipeline>) documentation.

## SuperComponents[​](<#supercomponents> "Direct link to SuperComponents")

To simplify your code, we have introduced [SuperComponents](</docs/supercomponents>) that allow you to wrap complete pipelines and reuse them as a single component. Check out their documentation page for the details and examples.
...
### Smart Pipeline Connections[​](<#smart-pipeline-connections> "Direct link to Smart Pipeline Connections")
...
See [Smart Pipeline Connections](</docs/smart-pipeline-connections>) for details and examples.

## Steps to Create a Pipeline Explained[​](<#steps-to-create-a-pipeline-explained> "Direct link to Steps to Create a Pipeline Explained")

Once all your components are created and ready to be combined in a pipeline, there are four steps to make it work:

  1. Create the pipeline with `Pipeline()`. This creates the Pipeline object.
  2. Add components to the pipeline, one by one, with `.add_component(name, component)`. This just adds components to the pipeline without connecting them yet. It's especially useful for loops as it allows the smooth connection of the components in the next step because they all already exist in the pipeline.
  3. Connect components with `.connect("producer_component.output_name", "consumer_component.input_name")`. At this step, you explicitly connect one of the outputs of a component to one of the inputs of the next component. This is also when the pipeline validates the connection without running the components. It makes the validation fast.
  4. Run the pipeline with `.run({"component_1": {"mandatory_inputs": value}})`. Finally, you run the Pipeline by specifying the first component in the pipeline and passing its mandatory inputs. Optionally, you can pass inputs to other components, for example: `.run({"component_1": {"mandatory_inputs": value}, "component_2": {"inputs": value}})`.
...
Once you create your pipeline, you can [visualize it in a graph](</docs/visualizing-pipelines>) to understand how the components are connected and make sure that's how you want them. You can use Mermaid graphs to do that.
...
[Edit this page](<https://github.com/deepset-ai/haystack/tree/main/docs-website/versioned_docs/version-2.28/concepts/pipelines.mdx>)

[PreviousSuperComponents](</docs/supercomponents>)[NextCreating Pipelines](</docs/creating-pipelines>)
  ...
  * [Data Flow](<#data-flow>)
    * [Smart Pipeline Connections](<#smart-pipeline-connections>)
  * [Steps to Create a Pipeline Explained](<#steps-to-create-a-pipeline-explained>)
