...
本文件是当前课程根目录的规则层。它吸收 `001/course-config.md` 中有价值的机制，但不继承旧学习进度和旧工程化内容。
...
- 外部深度学习实操教材、在线教程、框架文档、部署专题和旧代码样例。
- 旧学习记录中的代码实现、工程实操、部署流程和旧教程结论。
- `001/skill-map/` 中的旧工程化技能图谱。
- `其他资料/` 中未被当前教材路线明确解锁的材料。
- 教材正文没有讲过、且知识点指引没有要求的后文大块内容。

如果学习者主动问到禁止源内容，AI 应说明：这不属于当前 NNDL 主课，先不混入；可另开独立实操课程学习。
